...
public static class FileNameDateParser
{
    public static DateTime ExtractDateFromFileName(string filePath)
    {
        ...
        // Regex patterns for common date formats (added \. as separator option)
        string[] patterns =
        [
            @"\b\d{4}[-./]?\d{2}[-./]?\d{2}\b",       // 20250801 or 2025-08-01 or 2025.08.01
            @"\b\d{1,2}[-/\.]\d{1,2}[-/\.]\d{2,4}\b"  // 8-1-25 or 08/01/2025 or 08.01.2025
        ];
        ...
        foreach (var pattern in patterns)
        {
            ...
            if (match.Success)
            {
                ...
                // List of accepted formats (added dot variants)
                string[] formats =
                [
                    "yyyyMMdd",
                    "yyyy-MM-dd",
        ...
        return DateTime.Now; // no date found
    }
}
